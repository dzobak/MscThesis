...
# Hardcoded levels, should return the levels of scope or deepest scope level
            ...
            # agg = Aggregation()
            # print(data)
            # aggregated_log = agg.aggregate_log(log=log, level=data["level"], scope=data["scope_column"])
            aggregated_log = execute_aggregation(log, **data)
            return aggregated_log.get_extended_table().head(10).to_json(orient="records")
        else:
            return {
                "good": "niddce",
                "hithere": task
