...
def aggregate_events(log, **kwargs):
    ...
    log.events[special_columns['id_column']] =\
        log.events[special_columns['id_column']].astype(float)
    log.relations[special_columns['id_column']] =\
        log.relations[special_columns['id_column']].astype(float)
    ...
    col_func_map[special_columns['act_column']] =\
    ...
    agg_events = log.events.groupby([kwargs['scope_column'],
                                    pd.Grouper(key=special_columns['ts_column'],
    ...
    agg_events.sort_values(
        special_columns['id_column'], inplace=True, ignore_index=True)

    # change relations eid column
    value_mapping = agg_events.apply(
        get_values_mapping, axis=1, old_ids_column='old_ids', new_id_column=special_columns['id_column'])
    value_mapping = value_mapping.aggregate(concat_dicts)
    log.relations = map_ids(
        log.relations, special_columns['id_column'], value_mapping)
...
def aggregate_objects(log, **kwargs):
    ...
    value_mapping = agg_objs.apply(get_values_mapping, axis=1, old_ids_column=(
        special_columns['id_column'], 'setify'), new_id_column=(special_columns['id_column'], 'min'))
    value_mapping = value_mapping.aggregate(concat_dicts)
    log.relations = map_ids(
        log.relations, special_columns['id_column'], value_mapping)
